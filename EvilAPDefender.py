	...
	if cursor.rowcount > 0:
		...
		if cursor.rowcount > 0:
			...
			try:
			    ...
			    smtpObj = smtplib.SMTP(admin_smtp,587,"localhost",1)
			    print bcolors.OKBLUE + "\nSending ehlo message to SMTP server\n" + bcolors.ENDC
			    smtpObj.ehlo()
			    smtpObj.starttls()
			    smtpObj.ehlo()
			    print bcolors.OKBLUE + "\nLogin to SMTP server\n" + bcolors.ENDC
			    smtpObj.login(admin_smtp_username,admin_smtp_password)
			    print bcolors.OKBLUE + "\nSending the message ...\n" + bcolors.ENDC
			    smtpObj.sendmail(admin_smtp_username, admin_email, message)
			    #smtpObj.quit()
...
def Deauth(Dbssid,Dssid,Dchannel,Dtime):
	...
	Reset("INTF")
	mon_interface = Popen(["airmon-ng", "start", wireless_interface, Dchannel], stdout=PIPE).communicate()[0]
...
try:
    ...
    aps = {}
    sniff(iface=mon_iface, prn=insert_ap, count=50, store=False, lfilter=lambda p: (Dot11Beacon in p or Dot11ProbeResp in p))
